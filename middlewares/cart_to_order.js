      ...
      if (items.length) {
        items.map(async (item, index) => {
          ...
          if (productDetails.discount_id != undefined) {
            let discountDetail = await Discount.findById(
              productDetails.discount_id
            );
            discount_price =
              discountDetail.discount_type == "percent"
                ? (parseInt(discountDetail.discount_amount) / 100) *
                  productDetails.price
                : parseInt(discountDetail.discount_amount);
          }
