...
let findData = async (prop, val) => {
  if (prop === 'id')
    prop = '_id'
  return PaymentMethod.find({ [prop]: val })
    .then(resp => {
...
let findDataBy = (params) => {
  return PaymentMethod.find(params)
    .then(serialize);
}

let addData = (dataObj) => {
  return PaymentMethod.create(dataObj)
    .then(serialize);
}

let updateData = (id, dataObj) => {
  return PaymentMethod.findByIdAndUpdate(id, dataObj)
    .then(serialize);
}
