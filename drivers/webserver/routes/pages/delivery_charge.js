...
router.get('/delivery-charges',
    ...
    (req, res, next) => {
        res.render('pages/delivery-charge-list', {
            ...menuAccess.getProgram(req.user.role, "deliveryMenu.deliChargeSubMenu.list"), // admin may change on req.user => role
...
router.get('/delivery-charge/:id?',
    ...
    async (req, res, next) => {
        ...
        res.render('pages/delivery-charge-entry', {
            ...menuAccess.getProgram(req.user.role, "deliveryMenu.deliChargeSubMenu.entry"), // admin may change on req.user => role
            token: Handlers.generateTokenSign(config.jwt.credential.USERNAME),
            app: config.app,
            data: data
        });
