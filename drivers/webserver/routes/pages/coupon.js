...
router.get('/coupon/:id?',
    ...
    async (req, res, next) => {
        let data = {};
        if (req.params.id)
            data = await CouponsDb.findData('id', req.params.id);
        ...
        res.render('pages/coupon-entry', {
            ...menuAccess.getProgram(req.user.role, "marketingMenu.couponSubMenu.entry"), // admin may change on req.user => role
            token: Handlers.generateTokenSign(config.jwt.credential.USERNAME),
            app: config.app,
            data: data
        });
    ...
    .post('/coupon',
        (req, res, next) => {
            ...
            if (req.body.start_time && req.body.expire_time) {
                ...
                req.body.expire_time = req.body.expire_time.split("/").reverse().join("-");
            }
