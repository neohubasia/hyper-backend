...
router.get("/templates", connect.ensureLoggedIn(), (req, res, next) => {
...
router
  .get("/template/:id?", connect.ensureLoggedIn(), async (req, res, next) => {
    let data = {};
    if (req.params.id) data = await TemplatesDb.findData("id", req.params.id);

    res.render("pages/template-entry", {
      ...menuAccess.getProgram(
        req.user.role,
        "generalMenu.templateSubMenu.entry"
